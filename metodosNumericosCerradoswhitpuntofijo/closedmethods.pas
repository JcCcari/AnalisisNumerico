...
type
  TClosedMethods = class
    ...
    public
      ...
      function puntoFijo(x, gx: String ; a: Double; e: Double): TResult;
  end;
...
function TClosedMethods.puntoFijo(x, gx: String ; a: Double; e: Double): TResult;
var
  fx, fgx: TParseMath;
  xn, xn_1, eAbs: Double;
  i: integer;
begin
     ...
     fgx := TParseMath.create;
     fx.Expression := x ;
     fgx.Expression := gx ;

     fx.AddVariable('x',0); fx.Evaluate();
     fgx.AddVariable('x',0); fgx.Evaluate();

     fgx.NewValue('x',a);
     if ( abs(fgx.Evaluate()) > 1) then
     begin
         Result.result:= 'No Cumple -1< g"(x) < 1';
         exit;
     end;
     SetLength( Result.matrix,1,3);
     Result.matrix[0,0] := IntToStr(i);
     Result.matrix[0,1] := FloatToStr(a);
     Result.matrix[0,2] := '-';
     ...
     begin
          ...
          Result.matrix[i,0] := IntToStr(i);
          Result.matrix[i,1] := FloatToStr(xn);
          Result.matrix[i,2] := FloatToStr(eAbs);
